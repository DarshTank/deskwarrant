...
# Baked in at build time so a fresh install has nothing to type at all. This is
# a single-tenant deployment -- every agent talks to the same console -- so the
# URL is configuration, not a per-user secret. DESKWARRANT_CONSOLE_URL overrides
# it for development against a local console.
#
# _build_config.py is generated by the release workflow and is not in the repo,
# so a source checkout falls through to the empty default and asks once.
try:
    from _build_config import CONSOLE_URL as DEFAULT_CONSOLE_URL
except ImportError:
    DEFAULT_CONSOLE_URL = ""


def default_console_url() -> str:
    return (os.environ.get("DESKWARRANT_CONSOLE_URL") or DEFAULT_CONSOLE_URL).rstrip("/")
...
@dataclass
class AgentConfig:
    console_url: str = field(default_factory=default_console_url)
    ...
    def load(cls) -> "AgentConfig":
        ...
        return cls(
            # A blank saved value falls back to the build-time default rather
            # than pinning the agent to "no console".
